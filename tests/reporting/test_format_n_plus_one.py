...
# Three levels up, because this file sits in ``tests/reporting/``: the tests
# below chdir here so a call site renders relative, and the string they then
# assert is the path *from the repository root*.
...
def test_the_call_site_comes_before_the_sql(authors: list[Author]) -> None:
    ...
    assert __file__ in lines[0] or "tests/reporting/test_format_n_plus_one.py" in lines[0]
...
def test_a_call_site_under_the_working_directory_is_shortened(
    ...
    assert "from tests/reporting/test_format_n_plus_one.py:" in _one(capture)
